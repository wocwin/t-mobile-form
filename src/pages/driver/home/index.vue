    ...
    <scroll-view scroll-y class="driver-content">
      ...
      <div class="sudoku flex-box">
        <div
          ...
          @click.stop="toPage('autoQueue2')"
        >
          <div class="icon-wrap flex-box flex-ver">
            <t-icon src="/static/images/icon/report_today.svg" />
          </div>
          <div class="txt">今日上报</div>
        </div>
        <div
          class="sudoku-item self-help2 flex-box flex-ver flex-col"
        ...
        >
          <div class="icon-wrap flex-box flex-ver">
            <t-icon src="/static/images/icon/report_tom.svg" />
          </div>
      ...
      <div class="content flex-box">
        <div class="left flex-box flex-ver flex-col">
          <div class="text">配送中任务</div>
          <div class="num t-margin-top-6">{{taskCountObj.distributionCount||0}}</div>
        </div>
        <div class="right flex-box flex-ver flex-col">
          <div class="text">已完成任务</div>
          <div class="num t-margin-top-6">{{taskCountObj.finishCount||0}}</div>
        </div>
      </div>
    </scroll-view>
  </div>
...
export default {
  ...
  methods: {
    // 获取任务数量
    async taskCount() {
      const res = await this.$http('taskCount')
    ...
    toPage(type) {
      switch (type) {
        ...
        case 'autoQueue2':
          ...
          break
        case 'autoQueue3':
          this.$openWin(`/pages/autoQueue2/main?type=spare`)
          break
        case 'epidemicEast':
          this.$openWin(`/pages/driver/epidemic/main?doorType=0`)
          break
        case 'epidemicWest':
          this.$openWin(`/pages/driver/epidemic/main?doorType=1`)
          break
        default:
          break
      }
